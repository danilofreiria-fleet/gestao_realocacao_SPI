...
import { Search, CheckCircle, AlertTriangle, XCircle, Slash, Truck, ChevronLeft, ChevronRight, ArrowUpDown, Filter } from 'lucide-react';
...
export default function RotationTable({ filtrosGlobais = {} }) {
  ...
  }, [rawData, viewMode, dateRange, searchTerm, selectedModal, selectedStatus, sortConfig, filtrosGlobais]);
  ...
  useEffect(() => { setCurrentPage(1); }, [searchTerm, selectedModal, selectedStatus, itemsPerPage]);
  ...
  return (
    // Removido o limite de tela cheio (h-calc), para deixar o dashboard livre
    <div className="flex flex-col space-y-4 mt-6">
      ...
      <div className="bg-white dark:bg-[#1f232d] rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 p-4 shrink-0">
        <div className="flex flex-wrap items-end gap-x-4 gap-y-3">
          ...
          <div className="flex flex-col">
            <label className="text-[9px] font-black text-slate-400 uppercase mb-0.5 ml-1 flex items-center gap-1"><Filter size={10}/> Status (Período)</label>
            <select 
              className="bg-slate-50 dark:bg-[#15171e] border border-slate-200 dark:border-gray-700 rounded-lg py-1.5 px-3 text-xs font-bold text-[#113366] dark:text-white outline-none cursor-pointer hover:border-[#113366] transition-colors"
          ...
          <div className="flex flex-col">
            <label className="text-[9px] font-black text-slate-400 uppercase mb-0.5 ml-1">Referência Mês</label>
          ...
          <div className="flex bg-slate-100 dark:bg-gray-800 p-1 rounded-lg ml-auto border border-slate-200 dark:border-gray-700">
            <button onClick={() => setViewMode('month')} className={`px-4 py-1 rounded text-[10px] font-black uppercase tracking-wider transition-all ${viewMode === 'month' ? 'bg-[#113366] text-white shadow-sm' : 'text-slate-500 hover:text-[#113366]'}`}>Mês</button>
            <button onClick={() => setViewMode('range')} className={`px-4 py-1 rounded text-[10px] font-black uppercase tracking-wider transition-all ${viewMode === 'range' ? 'bg-[#113366] text-white shadow-sm' : 'text-slate-500 hover:text-[#113366]'}`}>Manual</button>
          </div>
        </div>
        ...
        {viewMode === 'range' && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-gray-800 animate-in fade-in slide-in-from-top-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-2">Filtro Customizado:</span>
      ...
      <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-[#113366] overflow-hidden flex flex-col relative">
        ...
        {/* 🔥 A MÁGICA ACONTECE AQUI: max-h-[55vh] - A tabela trava em 55% da altura da tela e rola internamente! */}
          ...
          <table className="w-full border-collapse text-center">
            <thead>
              <tr className="bg-[#113366] text-white tracking-widest text-[9px] xl:text-[10px]">
                {/* Cabeçalho fixo no TOPO e ESQUERDA */}
              ...
              {loading ? (
                ...
                paginatedRows.map((row) => (
                  <tr key={row.id} className="even:bg-slate-50 odd:bg-white dark:even:bg-gray-800/40 dark:odd:bg-[#15171e] hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors h-8">
                    {/* Coluna Condutor fixa na ESQUERDA */}
