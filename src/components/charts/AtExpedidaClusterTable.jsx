...
import { Search, MapPin, Layers, TrendingUp, ChevronLeft, ChevronRight, ChevronDown, Download, PieChart as PieChartIcon, Maximize2, X } from 'lucide-react';
import { ResponsiveContainer, ComposedChart, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, LabelList, PieChart, Pie, Cell, Legend } from 'recharts';

// 🔥 IMPORTAMOS O MAPA PARA FAZER O CRUZAMENTO DOS HUBS COM AS SUBREGIONAIS
import { MAPA_REGIONAL_COMPLETO } from '../../constants/regionais';

// CORES PADRONIZADAS PARA MODAIS
const MODAL_COLORS = {
  'UTILITÁRIO': '#113366',
  'PASSEIO': '#EE4D2D',
  'MOTO': '#D0011B',
...
const CustomTooltipRank = ({ active, payload, label, isModal = false }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-[#1f232d] p-3 rounded-lg shadow-xl border border-[#113366]">
        <p className="font-black text-[#113366] dark:text-[#EE4D2D] border-b border-slate-200 pb-2 mb-2">{label || payload[0].name}</p>
        <p className="font-black text-[#113366] dark:text-white">
           Total Rotas: <span className="text-lg text-[#EE4D2D]">{payload[0].value.toLocaleString('pt-BR')}</span>
        </p>
      </div>
    );
  }
...
export default function AtExpedidaClusterTable({ atExpedidaData, filtrosGlobais = {} }) {
  ...
  const [expandedChart, setExpandedChart] = useState(null); // 'clusters' | 'modais' | null
  ...
  const getHeatmapColor = (val) => {
    ...
    if (val <= 5) return 'bg-[#fff2cc] text-[#7f6000] dark:bg-[#fff2cc]/20 dark:text-yellow-200';
    if (val <= 15) return 'bg-[#ffe599] text-[#7f6000] dark:bg-[#ffe599]/20 dark:text-yellow-300';
    if (val <= 30) return 'bg-[#ffd966] text-[#7f6000] dark:bg-[#ffd966]/20 dark:text-yellow-400';
    if (val <= 50) return 'bg-[#f4b084] text-[#783f04] dark:bg-[#f4b084]/30 dark:text-orange-300';
    if (val <= 100) return 'bg-[#ed7d31] text-white dark:bg-[#ed7d31]/50 dark:text-orange-400';
    if (val <= 200) return 'bg-[#e06666] text-white dark:bg-[#e06666]/70 dark:text-red-300';
    if (val <= 500) return 'bg-[#cc0000] text-white dark:bg-[#cc0000]/80 dark:text-red-400';
    return 'bg-[#990000] text-white dark:bg-[#990000] dark:text-white';
  };

  function parseDateLocal(str) {
    const iso = parseUniversalDate(str);
    return iso ? new Date(`${iso}T12:00:00`) : null;
  }

  const matrix = useMemo(() => {
    if (!atExpedidaData || atExpedidaData.length < 2) {
      return { headers: [], rows: [], stationsUnicas: [], rankingClusters: [], rankingModais: [] };
    }

    // 🔥 ADICIONADO 'regional' NA DESTRUTURAÇÃO DOS FILTROS GLOBAIS
    ...
    atExpedidaData.slice(1).forEach(row => {
      const hub = String(row[1] || "").trim(); // Col B
      const modal = String(row[3] || "NÃO INFORMADO").trim().toUpperCase(); // Col D
      const cluster = String(row[4] || "SEM CLUSTER").trim(); // Col E
      const dataStr = String(row[5] || "").trim(); // Col F

      if (!hub || !dataStr) return;

      // 🔥 CRUZA O HUB DA ROW COM O MAPA DE REGIONAIS PARA ENCONTRAR A SUBREGIONAL (EX: SPO3)
      const subregParaFiltrar = MAPA_REGIONAL_COMPLETO[hub];

      // 🔥 SE TIVER REGIONAL SELECIONADA NO DASHBOARD E A ROW NÃO BATER, IGNORA
      if (regional.length > 0 && !regional.includes(subregParaFiltrar)) return;
      ...
      aggs[hub].valoresHub[chaveTempo] += 1;
      ...
      aggs[hub].clustersMap[cluster].valores[chaveTempo] += 1;
      ...
      if (!modalRankMap[modal]) modalRankMap[modal] = 0;
      modalRankMap[modal] += 1;
    });
    ...
    const rankingClusters = Object.entries(clusterRankMap)
       .map(([name, total]) => ({ name, total }))
       .sort((a, b) => b.total - a.total).slice(0, 10);

    const rankingModais = Object.entries(modalRankMap)
       .map(([name, value]) => ({ name, value }))
       .sort((a, b) => b.value - a.value);

    return { headers, rows, stationsUnicas: Array.from(stationsSet).sort(), rankingClusters, rankingModais };
  }, [atExpedidaData, viewMode, filtrosGlobais, searchTerm]);
  ...
  const exportarClusterCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", `Estudo_RotasExpedidas_${hubDownload.replace(/\s+/g, '_')}.csv`);
  ...
  const renderTopClustersChart = (isModal = false) => (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={matrix.rankingClusters} layout="vertical" margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
        ...
        <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 'bold', fill: '#113366' }} width={200} />
        <Tooltip content={<CustomTooltipRank />} cursor={{ fill: 'rgba(0,0,0,0.05)' }} />
        <Bar dataKey="total" name="Rotas" fill="#113366" radius={[0, 4, 4, 0]} barSize={24}>
          <LabelList dataKey="total" position="right" style={{ fill: '#113366', fontSize: 11, fontWeight: '900' }} />
        </Bar>
      </ComposedChart>
    </ResponsiveContainer>
  );

  const renderTopModaisChart = () => (
    <ResponsiveContainer width="100%" height="100%">
      <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
        <Pie
          data={matrix.rankingModais}
          dataKey="value"
          nameKey="name"
          cx="50%"
          cy="50%"
          ...
          labelLine={true}
        >
          {matrix.rankingModais.map((entry, index) => (
            <Cell key={`cell-${index}`} fill={MODAL_COLORS[entry.name] || MODAL_COLORS['NÃO INFORMADO']} />
          ))}
        </Pie>
        <Tooltip content={<CustomTooltipRank isModal={true} />} />
  ...
  return (
    <div className="flex flex-col gap-6 relative">
      ...
      <div className="bg-white dark:bg-[#1f232d] p-6 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-2">
            <TrendingUp className="text-[#113366] dark:text-blue-400" size={22} />
            <div>
              <h3 className="text-lg font-black text-[#113366] dark:text-white uppercase">Volume de Rotas Expedidas</h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Visão analítica de expedição por Cluster e Modal</p>
            </div>
          </div>
      ...
      {/* ÁREA INFERIOR: GRÁFICOS LADO A LADO COM SCROLL INTERNO E BOTÃO EXPANDIR */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* GRÁFICO 1: TOP 10 CLUSTERS OFENSORES */}
        {matrix.rankingClusters.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2">
                <TrendingUp className="text-[#113366] dark:text-blue-400" size={20} />
                <h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Top 10: Clusters mais Expedidos</h3>
              </div>
              <button onClick={() => setExpandedChart('clusters')} className="text-slate-400 hover:text-[#EE4D2D] transition-colors p-1" title="Expandir Gráfico">
                 <Maximize2 size={16} />
              </button>
            </div>
        ...
        {/* GRÁFICO 2: PROPORÇÃO DE FROTA (MODAIS) */}
        {matrix.rankingModais.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2">
                <PieChartIcon className="text-[#EE4D2D]" size={20} />
                <h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Proporção da Frota Expedida (%)</h3>
              </div>
              <button onClick={() => setExpandedChart('modais')} className="text-slate-400 hover:text-[#113366] dark:hover:text-blue-400 transition-colors p-1" title="Expandir Gráfico">
                 <Maximize2 size={16} />
              </button>
            </div>
            
            <div className="flex-1 w-full relative">
               {renderTopModaisChart()}
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 md:p-10">
          <div className="bg-white dark:bg-[#1f232d] w-full max-w-7xl h-full max-h-[90vh] rounded-2xl shadow-2xl flex flex-col p-6 md:p-8 relative">
            ...
            <button 
              ...
              className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-gray-800 hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/30 rounded-full text-slate-500 transition-colors z-10"
            >
              <X size={24} />
            </button>
            ...
            <div className="flex items-center gap-2 mb-6 border-b border-slate-200 dark:border-gray-700 pb-4">
              {expandedChart === 'clusters' ? (
                 <TrendingUp className="text-[#113366] dark:text-blue-400" size={28} />
              ) : (
                 <PieChartIcon className="text-[#EE4D2D]" size={28} />
              )}
              <h2 className="text-xl font-black text-[#113366] dark:text-white uppercase">
                {expandedChart === 'clusters' ? 'Visão Detalhada: Top Clusters Expedidos' : 'Visão Detalhada: Proporção da Frota (%)'}
              </h2>
            </div>

            <div className="flex-1 w-full overflow-y-auto custom-scrollbar pr-4">
              {expandedChart === 'clusters' ? (
                <div style={{ height: Math.max(500, matrix.rankingClusters.length * 60) }}>
                  {renderTopClustersChart(true)}
                </div>
              ) : (
