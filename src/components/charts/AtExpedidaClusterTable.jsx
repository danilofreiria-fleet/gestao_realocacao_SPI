...
import { Search, MapPin, Layers, TrendingUp, ChevronLeft, ChevronRight, ChevronDown, Download, PieChart as PieChartIcon, Maximize2, X, Loader2 } from 'lucide-react';
...
import { calcularMatrizExpedida } from '../../utils/motorEstudosCluster'; 
...
const MODAL_COLORS = {
  'UTILITÁRIO': '#113366', 'PASSEIO': '#EE4D2D', 'MOTO': '#D0011B', 'VAN': '#F59E0B', 'NÃO INFORMADO': '#94a3b8'
};
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-[#1f232d] p-3 rounded-lg shadow-xl border border-[#113366]">
        ...
        <p className="font-black text-[#113366] dark:text-white">Total Rotas: <span className="text-lg text-[#EE4D2D]">{payload[0].value.toLocaleString('pt-BR')}</span></p>
      </div>
    );
  }
  return null;
...
const currentRegional = localStorage.getItem("selectedRegional");
  ...
  const [matrix, setMatrix] = useState({ headers: [], rows: [], stationsUnicas: [], rankingClusters: [], rankingModais: [] });

  const stringFiltros = JSON.stringify(filtrosGlobais);
  const lenData = atExpedidaData?.length || 0;

useEffect(() => {
    let isMounted = true;
    const rodarMotor = async () => {
      setIsCalculating(true);
      // Passamos selectedModal e currentRegional para o motor
      const resultado = await calcularMatrizExpedida({ 
          atExpedidaData, filtrosGlobais, viewMode, currentRegional 
      });
      if (isMounted) {
        setMatrix(resultado || { headers: [], rows: [], stationsUnicas: [], rankingClusters: [], rankingModais: [] });
  ...
  const renderTopClustersChart = (isModal = false) => (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={matrix.rankingClusters} layout="vertical" margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
        ...
        <Bar dataKey="total" name="Rotas" fill="#113366" radius={[0, 4, 4, 0]} barSize={isModal ? 32 : 24}>
  ...
  const renderTopModaisChart = () => (
    <ResponsiveContainer width="100%" height="100%">
      <PieChart margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
        <Pie data={matrix.rankingModais} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={2} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(1)}%`} labelLine={true}>
          {matrix.rankingModais.map((entry, index) => <Cell key={`cell-${index}`} fill={MODAL_COLORS[entry.name] || MODAL_COLORS['NÃO INFORMADO']} />)}
        </Pie>
    ...
    <div className="flex flex-col gap-6 relative animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {matrix.rankingClusters.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2"><TrendingUp className="text-[#113366] dark:text-blue-400" size={20} /><h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Top 10: Clusters mais Expedidos</h3></div>
        ...
        {matrix.rankingModais.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2"><PieChartIcon className="text-[#EE4D2D]" size={20} /><h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Proporção da Frota Expedida (%)</h3></div>
              <button onClick={() => setExpandedChart('modais')} className="text-slate-400 hover:text-[#113366] dark:hover:text-blue-400 transition-colors p-1"><Maximize2 size={16} /></button>
            </div>
            <div className="flex-1 w-full relative">{renderTopModaisChart()}</div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 md:p-10">
          <div className="bg-white dark:bg-[#1f232d] w-full max-w-7xl h-full max-h-[90vh] rounded-2xl shadow-2xl flex flex-col p-6 md:p-8 relative">
            ...
            <div className="flex items-center gap-2 mb-6 border-b border-slate-200 dark:border-gray-700 pb-4">
              {expandedChart === 'clusters' ? <TrendingUp className="text-[#113366] dark:text-blue-400" size={28} /> : <PieChartIcon className="text-[#EE4D2D]" size={28} />}
              <h2 className="text-xl font-black text-[#113366] dark:text-white uppercase">{expandedChart === 'clusters' ? 'Visão Detalhada: Top Clusters Expedidos' : 'Visão Detalhada: Proporção da Frota (%)'}</h2>
            </div>
            <div className="flex-1 w-full overflow-y-auto custom-scrollbar pr-4">
              {expandedChart === 'clusters' ? <div style={{ height: Math.max(500, matrix.rankingClusters.length * 60) }}>{renderTopClustersChart(true)}</div> : <div className="h-full w-full min-h-[500px]">{renderTopModaisChart()}</div>}
            </div>
          </div>
        </div>
      )}
