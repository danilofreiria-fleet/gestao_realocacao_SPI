    ...
    return (
      <div className="bg-white dark:bg-[#1f232d] p-3 rounded-lg shadow-xl border border-[#113366]">
        ...
        <p className="font-black text-[#113366] dark:text-white">
           Total Recusas: <span className="text-lg text-[#EE4D2D]">{payload[0].value.toLocaleString('pt-BR')}</span>
        </p>
      </div>
    );
  }
...
export default function RecusasClusterTable({ recusasData, filtrosGlobais = {} }) {
  ...
  // Estado para controlar o Gráfico Expandido (Modal)
  const [expandedChart, setExpandedChart] = useState(null); // 'clusters' | 'motivos' | null
  ...
  const matrix = useMemo(() => {
    if (!recusasData || recusasData.length < 2) {
      return { headers: [], rows: [], stationsUnicas: [], rankingClusters: [], rankingMotivos: [] };
    }
    ...
    recusasData.slice(1).forEach(row => {
      ...
      const cluster = String(row[6] || "").trim() || "SEM CLUSTER";
      const dataStr = String(row[8] || "").trim();
    ...
    const rankingMotivos = Object.entries(motivoRankMap)
       .map(([name, total]) => ({ name, total }))
       .sort((a, b) => b.total - a.total).slice(0, 10);

    return { headers, rows, stationsUnicas: Array.from(stationsSet).sort(), rankingClusters, rankingMotivos };
  }, [recusasData, viewMode, filtrosGlobais, searchTerm]);
  ...
  // Funções para renderizar os gráficos (reutilizáveis para o Modal)
  const renderTopClustersChart = (isModal = false) => (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={matrix.rankingClusters} layout="vertical" margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
        ...
        {/* 🔥 YAxis expandido (width 250) para os nomes caberem inteiros */}
...
return (
    <div className="flex flex-col gap-6 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {matrix.rankingClusters.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2">
                <TrendingUp className="text-[#D0011B]" size={20} />
                <h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Top 10: Clusters com mais Recusas</h3>
              </div>
        ...
        {/* GRÁFICO 2: PARETO DE MOTIVOS DE RECUSA */}
        {matrix.rankingMotivos.length > 0 && (
          <div className="bg-white dark:bg-[#1f232d] rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 p-6 flex flex-col h-[400px]">
            <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-gray-800 pb-3">
              <div className="flex items-center gap-2">
                <PieChart className="text-[#113366] dark:text-blue-400" size={20} />
                <h3 className="font-black text-[#113366] dark:text-white uppercase text-sm">Distribuição por Motivo</h3>
              </div>
              <button onClick={() => setExpandedChart('motivos')} className="text-slate-400 hover:text-[#113366] dark:hover:text-blue-400 transition-colors p-1" title="Expandir Gráfico">
                 <Maximize2 size={16} />
              </button>
            </div>
            
            <div className="flex-1 w-full overflow-y-auto custom-scrollbar pr-2">
              <div style={{ height: Math.max(280, matrix.rankingMotivos.length * 55) }}>
                {renderTopMotivosChart()}
              </div>
            </div>
          </div>
        )}
          ...
          <div className="bg-white dark:bg-[#1f232d] w-full max-w-7xl h-full max-h-[90vh] rounded-2xl shadow-2xl flex flex-col p-6 md:p-8 relative">
            ...
            <div className="flex items-center gap-2 mb-6 border-b border-slate-200 dark:border-gray-700 pb-4">
              {expandedChart === 'clusters' ? (
                 ...
                 <PieChart className="text-[#113366] dark:text-blue-400" size={28} />
              )}
              <h2 className="text-xl font-black text-[#113366] dark:text-white uppercase">
                {expandedChart === 'clusters' ? 'Visão Detalhada: Top Clusters com mais Recusas' : 'Visão Detalhada: Distribuição por Motivo'}
              </h2>
            </div>

            <div className="flex-1 w-full overflow-y-auto custom-scrollbar pr-4">
              <div style={{ height: Math.max(500, (expandedChart === 'clusters' ? matrix.rankingClusters.length : matrix.rankingMotivos.length) * 60) }}>
                 {expandedChart === 'clusters' ? renderTopClustersChart(true) : renderTopMotivosChart(true)}
              </div>
            </div>
