...
export default function DeliverySuccess() {
  ...
  const processed = useMemo(() => {
    if (rawData.length < 2 || (selectedHubs.length === 0 && !searchTerm)) {
  ...
  // Filtragem dos itens da lista de opções com base no que foi digitado
    ...
    <div className="flex flex-col h-full gap-6">
      ...
      <div className="bg-white dark:bg-[#1f232d] p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 shrink-0 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            {/* Custom Multi-Select Dropdown com Campo de Busca embutido */}
            <div className="md:col-span-6 flex flex-col gap-2 relative" ref={dropdownRef}>
                ...
                {/* Caixa flutuante com barra de pesquisa interna */}
                {dropdownOpen && (
                  <div className="absolute top-[100%] left-0 w-full bg-white dark:bg-[#1f232d] border border-slate-200 dark:border-gray-700 rounded-xl mt-1 shadow-xl z-50 max-h-60 overflow-y-auto custom-scrollbar p-1 flex flex-col gap-0.5">
                    
                    {/* 🔥 CAMPO DE PESQUISA DO HUB (Fica fixo no topo da caixinha) */}
                    <div className="p-1 sticky top-0 bg-white dark:bg-[#1f232d] z-10 border-b border-slate-100 dark:border-gray-800">
        ...
        {showsEmptyState ? (
             <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50/80 dark:bg-gray-900/80 backdrop-blur-sm z-10 p-6">
                <div className="bg-white dark:bg-[#1f232d] p-8 rounded-2xl shadow-xl flex flex-col items-center text-center max-w-md border border-slate-200 dark:border-gray-700">
                    <AlertCircle size={44} className="text-[#EE4D2D] mb-4" />
                    <h3 className="text-lg font-black text-[#113366] dark:text-white uppercase mb-2">Por favor, selecione seu HUB</h3>
                    <p className="text-sm font-bold text-slate-500 dark:text-gray-400 leading-relaxed">
                        Escolha um ou mais **HUBs** no menu acima ou utilize o campo de busca por **ID do Motorista** para visualizar a tabela de notas.
                    </p>
                </div>
            </div>
        ) : (
            ...
            <table className="w-full border-collapse text-center">
                ...
                {processed.hubsData.length === 0 ? (
                    <tr><td colSpan={processed.colSemanas.length + 2} className="p-10 text-center font-bold text-slate-400">Nenhum dado encontrado para os filtros ativos.</td></tr>
                ) : (
